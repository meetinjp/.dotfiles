...
| ------------ | ---------------------------------------------------------------- | ----------------------- |
| `.agents/`   | Codex plugin marketplace metadata                               | discovered by Codex     |
| `bin/`       | `~/.local/bin/` scripts (`niri-screenshot`)                      | stowed                  |
| `claude/`    | Retained Claude Code compatibility config                        | run by `install.sh`     |
...
| `codex/`     | Codex defaults, global guidance, docs MCP + plugin installer     | run by `install.sh`     |
...
`install.sh` installs the Codex CLI with OpenAI's official standalone installer
when it is missing, then runs `codex/apply.sh`. Set `DOTFILES_UPDATE_CODEX=1`
when running `install.sh` to refresh an existing installation. The first
interactive `codex` launch handles ChatGPT/API authentication.

The patcher deliberately manages only portable defaults while preserving
Codex's live auth, sessions, project trust, and other machine-local state:

- GPT-5.6 Sol with `xhigh` reasoning and the pragmatic communication style.
- Vim composer mode, full-screen TUI, useful footer fields, and unfocused
  completion/approval notifications.
- A global `~/.codex/AGENTS.md` symlink with concise personal working
  agreements.
- OpenAI's official documentation MCP server.
- The native `caveman` Codex plugin, installed from this repo's dotfiles
  marketplace entry (which tracks the upstream plugin subdirectory).
- Zsh completions generated by the installed Codex CLI.

The older Claude Code patcher still runs for compatibility; Codex is the
primary managed agent. No Claude auth, history, cache, or live state is copied
into Codex.
...
`install.sh` symlinks every package into `~`, installs/configures Codex and its
native `caveman` plugin, then refreshes the retained Claude compatibility
configuration.
...
- `Super + Shift + arrows` — focus monitor in that direction; add `Ctrl` to move
...
- Niri starts on a blank screen with cursor only: Noctalia didn't launch. Open
  Ghostty with `Super+Q` and run `qs -c noctalia-shell &` to start it manually.
